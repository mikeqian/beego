...
func TestUrlFor2(t *testing.T) {
	...
	if handler.UrlFor("TestController.GetUrl", ":username", "mikeqian") != "/v1/mikeqian/edit" {
		Info(handler.UrlFor("TestController.GetUrl"))
		t.Errorf("TestController.List must equal to /v1/mikeqian/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/mikeqian", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "mikeqian" {
		t.Errorf("post func should mikeqian")
	}
}

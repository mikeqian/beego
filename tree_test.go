...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "mikeqian")
		obj, param := tr.Match(r.requesturl)
		if obj == nil || obj.(string) != "mikeqian" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "mikeqian")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "mikeqian")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "mikeqian")
	tr.AddRouter("/shop/:sd/account", "mikeqian")
	t3 := NewTree()
